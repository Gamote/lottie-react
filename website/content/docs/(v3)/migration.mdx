...
- **Only what you import ships.** `Lottie` costs about 4.4 kB gzipped and `useLottie` about 3.4 kB, with per-import budgets enforced by the build. v2 shipped one bundled file, so every consumer carried all of it.
...
- **The light engine is an entry point.** `LottieLight` and `useLottieLight` render with `lottie_light`, and importing them never pulls the full engine.

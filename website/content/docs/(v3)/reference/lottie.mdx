...
Without children the component renders exactly one element, the display itself, so `<Lottie lottieRef={handle} />` is the whole cost of a command-driven animation.
Reach for [the hook](/docs/animation/the-instance) when you need a value (`state`, `error`, `playableFrames`) or a layout of your own.
